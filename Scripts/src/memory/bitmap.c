...
void bitmap_init(bitmap_t *bmp, uintptr_t base_addr, uint64_t free_ram_bits)
{
    ...
    write_to_terminal("Base Address of BITMAP: %llx\n",WHITE,base_addr);
    write_to_terminal("Word Count of BITMAP: %u\n",WHITE,bmp->word_cnt);
    write_to_terminal("Total Pages Available: %u\n",WHITE,bmp->total_pages);

    //memset to 0
    for (uint32_t i = 0; i < bmp->word_cnt; i++) bmp->map[i] = 0;
    
    write_to_terminal("RESERVED PAGES: %u\n",WHITE,bitmap_pages);
    ...
    {
        write_to_terminal("Allocation of Bitmap Successfull!\n", WHITE);
    }
    else
    {
        write_to_terminal("Allocation of Bitmap Failed!\n", WHITE);
        write_to_terminal("Not enough memory to store the BITMAP!\n", WHITE);
    }
}